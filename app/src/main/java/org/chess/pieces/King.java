...
public class King extends Piece {
    ...
    public static Collection<Move> calculateMoves(Collection<King> kings, Function<Pos, Piece> getPiece,
        ...
        for (King king : kings) {
            ...
            Predicate<Pos> patchedDangerMap = pos -> {
                var kingAttaking = validSimpleMoves.get(pos);
                if (dangerMap.test(pos) || (kingAttaking != null && kingAttaking != king))
                    return true;
                return false;
            };
    ...
    private Collection<Pos> getSimpleMoves(Function<Pos, Piece> getPiece, Function<Piece, Pos> getPos,
            Predicate<Pos> dangerMap) throws PieceNotInBoard {
        ...
        for (int[] pos : possibleMoves) {
            try {
                ...
                if ((pieceInPos == null || pieceInPos.color != color) && !dangerMap.test(movementPos))
    ...
    private Collection<Move> getCastlingMoves(Function<Pos, Piece> getPiece, Function<Piece, Pos> getPos,
            Predicate<Pos> dangerMap,
        ...
        if (!movedBefore.test(kingSideRook)) {
            var move = kingsideCaslte(row, column, dangerMap, getPiece);
            if (move != null)
                moves.add(move);
        }

        if (!movedBefore.test(queenSideRook)) {
            var move = queensideCaslte(row, column, dangerMap, getPiece);
            if (move != null)
                moves.add(move);
        }
    ...
    private Move kingsideCaslte(int row, int col, Predicate<Pos> dangerMap, Function<Pos, Piece> getPiece) {
        Pos firstSquare = PieceType.KINGSIDE_BISHOP.initialPos(color);
        Pos secondSquare = PieceType.KINGSIDE_KNIGHT.initialPos(color);
    ...
    private Move queensideCaslte(int row, int col, Predicate<Pos> dangerMap, Function<Pos, Piece> getPiece) {
        Pos firstSquare = PieceType.QUEEN.initialPos(color);
        Pos secondSquare = PieceType.QUEENSIDE_BISHOP.initialPos(color);
        Pos thirdSquare = PieceType.QUEENSIDE_KNIGHT.initialPos(color);
